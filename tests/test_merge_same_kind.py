"""Regression coverage for merge_overlapping_same_kind_elements (commit
...
from workflow.element_merging import merge_overlapping_same_kind_elements
...
def test_overlapping_same_kind_same_column_merge():
    ...
    out = merge_overlapping_same_kind_elements([a, b])
    assert len(out) == 1
    merged = out[0]
...
def test_transitive_chain_collapses_to_one():
    ...
    out = merge_overlapping_same_kind_elements([a, b, c])
    assert len(out) == 1
...
def test_unrelated_element_left_untouched():
    ...
    out = merge_overlapping_same_kind_elements([a, b, other])
    assert len(out) == 2
