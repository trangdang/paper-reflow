...
def test_narrow_fragment_not_pulled_into_complete_neighbor_across_gutter():
    ...
    out = _complete_undersized_elements([frag, complete], CONTENT_WIDTH)
    assert len(out) == 2
...
def test_two_narrow_halves_across_gutter_merge_to_spanning():
    ...
    out = _complete_undersized_elements([left, right], CONTENT_WIDTH)
    assert len(out) == 1
...
def test_same_column_fragments_merge_without_forcing_spanning():
    ...
    out = _complete_undersized_elements([top, bottom], CONTENT_WIDTH)
    assert len(out) == 1
...
def test_isolated_small_element_left_standalone():
    ...
    out = _complete_undersized_elements([lone, far], CONTENT_WIDTH)
    assert len(out) == 2
